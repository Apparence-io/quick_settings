...
data class Tile (
  ...
  companion object {
    ...
    fun fromList(list: List<Any?>): Tile {
      ...
      return Tile(label, tileStatus, contentDescription, stateDescription, drawableName, subtitle)
    }
  }
  fun toList(): List<Any?> {
    return listOf<Any?>(
...
interface QuickSettingsInterface {
  fun addTileToQuickSettings(title: String, drawableName: String, callback: (AddTileResult) -> Unit)
  fun startBackgroundIsolate(pluginCallbackHandle: Long, onStatusChangedHandle: Long?, onTileAddedHandle: Long?, onTileRemovedHandle: Long?)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: QuickSettingsInterface?) {
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.QuickSettingsInterface.addTileToQuickSettings", codec)
        if (api != null) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.QuickSettingsInterface.startBackgroundIsolate", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            var wrapped = listOf<Any?>()
            try {
              val args = message as List<Any?>
              val pluginCallbackHandleArg = args[0].let { if (it is Int) it.toLong() else it as Long }
              val onStatusChangedHandleArg = args[1].let { if (it is Int) it.toLong() else it as? Long }
              val onTileAddedHandleArg = args[2].let { if (it is Int) it.toLong() else it as? Long }
              val onTileRemovedHandleArg = args[3].let { if (it is Int) it.toLong() else it as? Long }
              api.startBackgroundIsolate(pluginCallbackHandleArg, onStatusChangedHandleArg, onTileAddedHandleArg, onTileRemovedHandleArg)
...
interface QuickSettingsBackgroundInterface {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: QuickSettingsBackgroundInterface?) {
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.QuickSettingsBackgroundInterface.onInitialized", codec)
        if (api != null) {
...
class QuickSettingsBackgroundToDart(private val binaryMessenger: BinaryMessenger) {
  ...
  fun onTileClicked(userCallbackHandleArg: Long, tileArg: Tile, callback: (Tile?) -> Unit) {
    val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.QuickSettingsBackgroundToDart.onTileClicked", codec)
  ...
  fun onTileAdded(callbackHandleArg: Long, tileArg: Tile, callback: (Tile?) -> Unit) {
    val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.QuickSettingsBackgroundToDart.onTileAdded", codec)
  ...
  fun onTileRemoved(callbackHandleArg: Long, callback: () -> Unit) {
    val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.QuickSettingsBackgroundToDart.onTileRemoved", codec)
